    ...
    internal sealed class PcanLinHardware
    {
        ...
        private string TryConnect()
        {
            ...
            try
            {
                ...
                // 连接模式按通道 Mode：Master=modMaster（可发 Header/跑调度），Slave=modSlave（只监听+应答）。
                // Slave 模式下 Master/HeaderOnly 发送项由 ValidateTransmitPlan 在连接前拒绝。
                string modeNotice = _cfg.GetHardwareModeNotice();
                if (modeNotice.Length > 0) LinDebugLog.Write("[CONN] 警告: " + modeNotice);
                LinPlHardwareMode plMode = _cfg.GetHardwareMode() == LinNodeMode.Slave
                    ? LinPlHardwareMode.modSlave : LinPlHardwareMode.modMaster;
                err = LinPlApi.InitializeHardware(_client, _hw, plMode, (ushort)_cfg.Baudrate);
                LinDebugLog.Write("[CONN] InitializeHardware mode=" + plMode + " baud=" + _cfg.Baudrate + " → err=" + err);
                ...
                if (err != LinPlError.errOK && err != LinPlError.errWrongParameterType)
                {
                    if (err == LinPlError.errUnknown || err == LinPlError.errManagerNotResponding)
                    {
                        ...
                        LinPlError reinitErr = LinPlApi.InitializeHardware(_client, _hw, plMode, (ushort)_cfg.Baudrate);
        ...
        /// 通道硬件模式为 Master 时门禁拒绝：PCAN Master 下的总线唤醒走 UI 明确
