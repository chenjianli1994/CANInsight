...
namespace PCAN_Client
{
    ...
    public static class IdFilterRule
    {
        /// <summary>解析筛选文本（逗号/空格分隔），填充精确ID集合与通配符模式列表</summary>
        public static void Parse(string text, HashSet<uint> exact, List<(uint mask, uint value, uint maxId)> wildcards)
        {
            exact.Clear();
            wildcards.Clear();
        ...
        public static bool Match(uint id, HashSet<uint> exact, List<(uint mask, uint value, uint maxId)> wildcards)
        {
    ...
    public class BaseParamter
    {
        ...
        public static readonly string softVersion = "V3.03.05 -- 2026-09-10";
